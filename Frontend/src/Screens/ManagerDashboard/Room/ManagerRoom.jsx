          ...
          {employees.length > 0 ? (
            employees.map((employee, index) => (
              <div className="employeeManager-card" key={index}>
                ...
                <h3 className="performance bad">Bad performance</h3>
                <p><strong>performace: </strong> {employee.performance}</p>
                <div className="task-info">
                  <p><strong>Accepted:</strong> {employee.totalAcceptedTasks}</p>
                  <p><strong>Rejected:</strong> {employee.totalRejectedTasks}</p>
                  <p><strong>Completed:</strong> {employee.totalCompletedTasks}</p>
                  <p><strong>Pending:</strong> {employee.totalPendingTasks}</p>
                  <p><strong>Failed:</strong> {employee.totalFailedTasks}</p>
                </div>
              </div>
            ))
          ) : (
